 ...
 */
export function detectPresetType(
  ...
  if (
    "content" in data &&
    "post_history" in data &&
    Object.keys(data).length <= 5
  )
    return "sysprompt";

  // Reasoning: has prefix + suffix + separator
  if ("prefix" in data && "suffix" in data && "separator" in data)
    return "reasoning";

  // TextGen: has temperature_last or sampler_priority
  if ("temperature_last" in data || "sampler_priority" in data)
    return "textgen";

  // NovelAI: has phrase_rep_pen or repetition_penalty_frequency
  if ("phrase_rep_pen" in data || "repetition_penalty_frequency" in data)
    return "novel";

  // KoboldAI: has temp + rep_pen + sampler_order (broadest, last)
  if ("temp" in data && "rep_pen" in data && "sampler_order" in data)
    return "kobold";
