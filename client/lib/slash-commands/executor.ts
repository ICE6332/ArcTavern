...
export async function executePipeline(
  ...
  for (const cmd of pipeline.commands) {
    ...
    try {
      ...
      // Commands that trigger generation set shouldSendMessage
      if (
        cmd.name === "gen" ||
        cmd.name === "impersonate" ||
        cmd.name === "continue"
      ) {
        shouldSendMessage = false // these commands handle sending internally
      }
    } catch (err) {
  ...
  return { result: lastResult, shouldSendMessage }
}
