...
vi.mock("@/stores/connection-store", async () => {
  const actual = await vi.importActual<typeof import("@/stores/connection-store")>(
...
vi.mock("@/stores/prompt-manager-store", async () => {
  const actual = await vi.importActual<typeof import("@/stores/prompt-manager-store")>(
    "@/stores/prompt-manager-store",
  );
